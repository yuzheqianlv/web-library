...
pub fn create_monolithic_document(
    ...
    let mut target_url = match Url::parse(target_str) {
...
mod tests {
    ...
    fn test_detect_media_type_by_file_name_multiple_dots() {
        assert_eq!(
            detect_media_type_by_file_name("jquery.min.js"),
            "text/javascript"
        );
        assert_eq!(
            detect_media_type_by_file_name("bootstrap.min.css"),
            "text/css"
        );
    }
    ...
    fn test_parse_content_type_complex() {
        let (media_type, charset, is_base64) =
            parse_content_type("text/html; charset=iso-8859-1; base64");
        assert_eq!(media_type, "text/html");
