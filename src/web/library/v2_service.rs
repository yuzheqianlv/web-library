...
use super::cache::{CacheConfig, StatsCacheWithMetrics};
use super::v2_types::*;
...
impl LibraryServiceV2 {
    ...
    /// 获取库记录列表（V2版本）
    pub async fn get_library_records(
        &self,
    ...
    fn count_applied_filters(&self, query: &LibraryListQuery) -> i32 {
        ...
        if query.source_lang.is_some() {
            count += 1;
        }
        if query.target_lang.is_some() {
            count += 1;
        }
    ...
    pub async fn get_library_stats(
        &self,
        query: &StatsQuery,
    ) -> Result<LibraryStatsResponse, String> {
        ...
        self.stats_cache
            .set_with_ttl(query.detailed, query.days, stats.clone(), ttl)
            .await;
    ...
    /// 计算库统计信息（内部方法）
    async fn compute_library_stats(
        &self,
        query: &StatsQuery,
    ) -> Result<LibraryStatsResponse, String> {
        // 构建时间过滤条件
        ...
        if let Some(days) = query.days {
            ...
            base_filter.insert(
                "created_at",
                doc! { "$gte": BsonDateTime::from_chrono(since) },
            );
        }
        ...
        let today_start = chrono::Utc::now()
            ...
            .and_utc();
        let today_filter =
            doc! { "created_at": { "$gte": BsonDateTime::from_chrono(today_start) } };
        let today = self
            .collection
    ...
    async fn get_domain_stats(
        ...
        let mut domains: Vec<DomainStat> = domain_stats
            .into_iter()
            .map(|(domain, (count, total_size, success_count))| DomainStat {
                domain,
                count,
                total_size,
    ...
    pub async fn get_today_stats(&self) -> Result<TodayStats, String> {
        ...
        let today_pending =
            self.collection
                .count_documents(today_pending_filter)
                .await
        ...
        let today_error = self
            .collection
            .count_documents(today_error_filter)
            .await
        ...
        let mut today_languages: std::collections::HashMap<String, i64> =
        ...
        Ok(TodayStats {
            ...
            success_rate: if today_total > 0 {
                today_success as f64 / today_total as f64
            } else {
                0.0
            },
        })
    ...
    pub async fn create_v2_indexes(&self) -> Result<(), String> {
        ...
        let indexes = vec![
            ...
            IndexModel::builder()
                ...
                .options(
                    IndexOptions::builder()
                        .unique(true)
                        .name("url_lang_unique_v2".to_string())
                        .build(),
                )
                .build(),
            // 创建时间索引（用于排序和日期筛选）
            IndexModel::builder()
                ...
                .options(
                    IndexOptions::builder()
                        .name("created_at_desc_v2".to_string())
                        .build(),
                )
                .build(),
            // 标题索引（用于搜索）
            IndexModel::builder()
                ...
                .options(
                    IndexOptions::builder()
                        .name("title_url_text_v2".to_string())
                        .build(),
                )
                .build(),
            // 语言索引
