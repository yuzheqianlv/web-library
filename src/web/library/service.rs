...
impl LibraryService {
    ...
    /// 添加HTML文档到数据库
    pub async fn add_html_document(&self, doc: crate::web::types::CachedHtml) -> Result<bson::oid::ObjectId, String> {
    ...
    /// 更新HTML文档
    pub async fn update_html_document(&self, id: bson::oid::ObjectId, doc: crate::web::types::CachedHtml) -> Result<(), String> {
    ...
    /// 获取库数据
    pub async fn get_library_data(&self, request: &LibraryRequest) -> Result<LibraryResponse, String> {
        use futures::stream::TryStreamExt;
        ...
        for (domain_name, (count, total_size)) in domain_stats {
            // 如果有域名过滤，检查是否匹配
            if let Some(ref domain_filter) = request.domain {
                if !domain_name.to_lowercase().contains(&domain_filter.to_lowercase()) {
                    continue;
                }
            }
    ...
    /// 获取域名详情
    pub async fn get_domain_details(&self, request: &DomainDetailsRequest) -> Result<DomainDetailsResponse, String> {
        use futures::stream::TryStreamExt;
    ...
    pub async fn get_library_stats(&self) -> Result<LibraryStats, String> {
        ...
        let expired_urls = self.collection.count_documents(expired_filter).await
    ...
    /// 删除特定URL
    pub async fn delete_url(&self, url: &str, source_lang: &str, target_lang: &str) -> Result<bool, String> {
        let filter = doc! {
            "url": url,
    ...
    pub async fn create_indexes(&self) -> Result<(), String> {
        ...
        let indexes = vec![
            ...
            IndexModel::builder()
                ...
                .options(IndexOptions::builder().unique(true).name("url_lang_unique".to_string()).build())
                .build(),
            IndexModel::builder()
                .keys(doc! { "created_at": -1 })
                .options(IndexOptions::builder().name("created_at_desc".to_string()).build())
                .build(),
            IndexModel::builder()
            ...
            IndexModel::builder()
                ...
                .options(IndexOptions::builder().name("size_bytes_desc".to_string()).build())
                .build(),
        ];
